...
- `manifestPath` (optional): a served JSON index generated by
  `generate_thumbnails.py`. It follows the same URL and CORS rules as `path`;
  its adjacent chunk directory must also be served.
- `discoveryMode` (optional, default `"auto"`): controls how FolderFrame finds
  media. `"auto"` tries `manifestPath` first and falls back to HTML directory
  listings, `"directory"` uses directory listings without requesting a
  manifest, and `"manifest"` requires a valid published manifest and never
  attempts directory discovery. `"manifest"` requires `manifestPath`.
- `scanCache` (optional, default `false`): stores a lean parsed-directory manifest
  ...
  It is ignored in strict `"manifest"` mode.
...
listings with child links. FolderFrame ignores parent links and links outside
the directory being scanned. Strict `"manifest"` sources do not require
...
For a very large tree, configure a generated index alongside optional previews:
...
Create both outputs in one pass, or update only the index:
...
The helper writes the small root index plus a `library.d/` directory containing
one JSON chunk per top-level media folder. Known directories are compared by
directory mtime; unchanged records are reused, while changed directories are
listed again to capture additions, deletions, and renames. The browser receives
media path, file mtime, size, and available thumbnail path without per-file
metadata requests. The output may live in Docker/Unraid appdata, but both the
JSON file and chunk directory must be readable through the web server.

The persistent index is optional optimization data, not a database. Its minimum
contract is a supported `version`, a valid root directory record, and valid
descriptors for any referenced chunks. A valid empty root is allowed. In
`"auto"` mode, missing or invalid files, unreadable chunks, and CORS failures are
logged and fall back to normal HTML directory scanning. In strict `"manifest"`
mode, those failures produce an actionable published-index error and never attempt
