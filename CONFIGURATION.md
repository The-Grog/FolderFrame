...
The persistent index is optional optimization data, not a database. Missing or
invalid files, unreadable chunks, and CORS failures are logged and fall back to
normal HTML directory scanning. Refresh Folder bypasses both persistent and
browser-local manifests. A scheduled helper run keeps the served index current;
the browser cannot write server appdata.
